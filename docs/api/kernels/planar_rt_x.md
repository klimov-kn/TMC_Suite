...
> Пакет **TMC Suite**. Счётное ядро для электродинамического расчёта планарных структур в **X-моде** во временно́й области. Выходной файл: `tmc_rtx.exe`.
> Язык документации: русский. Все сигнатуры приведены по исходникам из `src/kernels/PlanarRT_X/` и общим заголовкам из `src/Include/`.
...
> ⚠️ Отличия реализованы **прямо в коде копий**, а не через `#ifdef`: поиск `#ifdef ELECTRON_Q___` / `#ifdef CTMCRTH_INDANBLCK_*` по всем исходникам не находит ни одного вхождения.
...
> ⚠️ Правило проекта: эти макросы задаются **только в свойствах проекта X-моды** (C/C++ → Preprocessor → Preprocessor Definitions) и **не добавляются в общие заголовки**.
...
X-вариант `TmcRTHNodeDiel.cpp` дополнительно распознаёт в секции TOPOLOGY ключевые слова `RECT_STAT_Y`, `CIRCLE_STAT_Y`, `POLYGON_STAT_Y`, `FILE_Y` (строковые константы `CTMCRTH_INDANBLCK_*Y` из `Tmcgrviw.h`). H-вариант этих блоков не обрабатывает. Назначение суффикса `_Y` (вероятно, задание блока проводимостью Y) — см. чек-лист, §7.
...
1. **Физика X-моды:** что именно считается (упоминались компоненты E_z/H_x/H_y; в коде X используются заряд/масса электрона — плазма/электронный поток?). Подтвердить корректную формулировку для документации.
2. **Шесть макросов в настройках проекта:** в текущем коде нет ни одного `#ifdef` по этим именам, а сами имена определены в общих заголовках (`Typerth.h`, `Tmcgrviw.h`) как константы. Нужны ли макросы в `PreprocessorDefinitions` для текущей редакции кода (и не вызывают ли предупреждений C4005), или это требование старой редакции заголовков?
3. **Проводимости узла:** физический смысл `y1..y7` и шести значений `prYNode[0..5]` (§3.2).
4. **Блоки `RECT_STAT_Y` / `CIRCLE_STAT_Y` / `POLYGON_STAT_Y` / `FILE_Y`:** что задаёт суффикс `_Y` и чем такие блоки отличаются от обычных `*_STAT`.
5. **`DistributionIntegrated` закомментирован:** диаграмма направленности в X-моде не поддерживается принципиально или временно?
6. **Размер массивов `6*nNumNode`:** почему множитель 6 (а не 8 — по числу компонент напряжения)? Связано ли с 6 проводимостями?
7. **Удаление `rUo`:** как в X-моде выводится рассеянное напряжение входов (в H для этого служило поле `rUo`)?
...
- 📸 НУЖНА СХЕМА: структура `sTmcRTHNodeDielOne` в X-моде (8 компонент напряжения)
