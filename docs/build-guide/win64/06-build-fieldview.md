...
MFC‑приложение (статическая MFC, MultiByte). Линкуется с OpenGL (`opengl32`, `glu32`)
и TMC‑библиотеками **sfile95**, **prepr**, **exprint**, **TMCIndan**, **TMCLibError**.
...
> (**П‑1**, **П‑2**), плюс общая правка **Ф4‑1**. Все — в `08-porting-changes.md`.
...
- Библиотеки `opengl32.lib`, `glu32.lib` — для платформы `x64` берутся из **`um\x64`**
  части SDK. Имя файла остаётся `opengl32.lib` (отдельной «OpenGL64» нет); нужную
  разрядность Visual Studio выбирает сама по платформе сборки.
...
> ⚠️ `glaux.lib` в списке быть **не должно** — GLAUX удалён из современного SDK
> (правка **Ф4‑1**).
...
**Настройки сборки (`.vcxproj`/`.sln`):** `FldView.vcxproj` переписан по образцу ядер
(toolset **v145**, 4 конфигурации Win32/x64 × Debug/Release, подключён
...
- **П‑1** (64 бит): в `TMCDialogPropet.h`/`.cpp` `int DoModal()` → `INT_PTR DoModal()`
  (override `CPropertySheet::DoModal`, ошибка `C2555`).
- **П‑2** (64 бит): в `FldViewView.h`/`.cpp` `OnTimer(UINT)` → `OnTimer(UINT_PTR)`
  (`C2440`/`C2737`).

> 🔧 Здесь применены изменения для 64 бит — см. `08-porting-changes.md`, пункты **П‑1**,
> **П‑2** (раздел «Фаза 4») и общая **Ф4‑1**.

> Примечание: файлы `TmcRTH_BlockList1.cpp/.h` (жёсткий путь `z:\work\...`) в проект НЕ
> включены.
...
3. Проверить линковку (6.1): `opengl32.lib`, `glu32.lib`, `sfile95.lib`, `prepr.lib`,
...
64‑битная сборка завершена. При проблемах — **`07-troubleshooting.md`**. Полный журнал
изменений ради 64 бит — **`08-porting-changes.md`**.
