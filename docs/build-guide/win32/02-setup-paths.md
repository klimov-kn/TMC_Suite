...
![Меню View → Property Manager](../../screenshots/build/vs-open-property-manager.png)
...
![Контекстное меню «Add Existing Property Sheet»](../../screenshots/build/vs-add-property-sheet.png)
...
`dist\win32\lib`, а, например, в `d:\work\lib`, и в окне Output появляется предупреждение
**MSB8012**:
...
2. **Configuration Properties → General → Output Directory** — если стоит жёсткий путь
   (`.\Release\` и т. п.), раскройте стрелку справа → **`<inherit from parent or project
   defaults>`** (вернётся `$(OutDir)` из property sheet → `dist\win32\lib`).
3. **Configuration Properties → Librarian → General → Output File** — если стоит жёсткий путь
   (`d:\work\lib\sfile95.lib` и т. п.), раскройте стрелку справа → **`<inherit from parent or
   project defaults>`** (станет `$(OutDir)$(TargetName)$(TargetExt)`).
4. **OK**. После пересборки `.lib` ляжет в `dist\win32\lib`, предупреждение MSB8012 исчезнет.

![Output Directory и Output File со значением «inherit…» (General и Librarian → General)](../../screenshots/build/vs-lib-output-inherit.png)

> ⚠️ Проверяйте это для **каждого** проекта (все 6 библиотек, программы): жёсткие пути
> `d:\work`, `e:\work`, `.\Release\` могли остаться в любом из них и в любой конфигурации.
...
![Выпадающие списки «Release» и «Win32»](../../screenshots/build/vs-config-release-win32.png)
...
![Configuration Manager с включённой галочкой Build для Release/Win32](../../screenshots/build/vs-configuration-manager-build.png)
