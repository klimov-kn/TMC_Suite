...
``build\ExeOutput.props``; имя exe — через ``<TargetName>`` (``tmc_rth``/``tmc_rtx``);
убраны жёсткие пути ``E:\WORK…``, ``c:\tmc\exe\…``. Убрана опция ``/MACHINE:I386``
(несовместима с x64). Исправлены пути к общим исходникам ``..\..\viewers\Tmcrtout\…``.
У X-моды задан свой ``ProjectGuid``, чтобы каталоги сборки H и X не пересекались.

**🔧 Правка кода П-3 (нужна и для win32, и для win64):**

- В ``kernels\PlanarRT_H\PL_GLFUN.CPP`` и ``kernels\PlanarRT_X\Pl_glfun.cpp``, функция
  ``s_tone``: интринсики прямого доступа к портам ``outp``/``inp`` удалены из
  современного компилятора (``error C3861``); на x64 прямой доступ к портам в принципе
  невозможен. Ветка Windows 95 заменена на ``Beep(freq, time)``. Ветка на современных
  ОС недостижима. Математика расчётов не затронута.

.. note::

   🔧 Здесь применено изменение — см. ``08-porting-changes``, пункт **П-3**.
...
4. Добавить шесть макросов (через ``;``), сохранив ``%(PreprocessorDefinitions)`` в
   конце::
...
В ``<КОРЕНЬ>\dist\win64\bin`` добавились ``tmc_rth.exe``, ``tmc_rtx.exe``. Прогоните
оба на тестах ``samples\SAMPLE_R``.

.. warning::

   Кнопка «Статистика» (Shift+F4): ранее крашила программу (Баг #1), сейчас
   **исправлена** (см. ``07-troubleshooting``). 64-битные сборки ядер пересобраны с
   исправлением.

Переходите к **``06-build-fieldview``**.
